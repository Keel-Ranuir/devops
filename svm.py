...
def rbf(x_1, x_2, sigma=1.):
    ...
    '''
    distances = np.exp(-scipy.spatial.distance.cdist(x_1.numpy(), x_2.numpy(), 'sqeuclidean') / (2 * sigma**2))
...
class SVM(BaseEstimator, ClassifierMixin):
    ...
    def predict_scores(self, batch):
        with torch.no_grad():
            ...
            return K @ self.betas + self.bias

    def predict(self, batch):
        scores = self.predict_scores(batch)
        answers = np.full(scores.shape, -1, dtype=np.int64)
        answers[scores > 0] = 1
        return answers